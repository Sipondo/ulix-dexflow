...
class Avgbasedef(BaseMoveEffect):
    def after_move(self):
        ...
        for stat in ("Defense", "Special Defense"):
            avg_stat = (user_stats[STATMAP[stat]] + target_stats[STATMAP[stat]])//2
            if swap_effects := [x for x in self.scene.get_effects_on_target(self.move.user) if x.name == "Statswap"]:
            ...
            if swap_effects := [x for x in self.scene.get_effects_on_target(self.move.target) if x.name == "Statswap"]:
                swap_effects[0].update(stat, avg_stat)
            else:
