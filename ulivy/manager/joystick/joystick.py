...
class Joystick(Widget):
    ...
    def _update_outlines(self, size):
        self._outer_line_width = (
            (self.outer_line_width * size)
            if (self.outer_line_width)
            else (OUTLINE_ZERO)
        )
        self._inner_line_width = (
            (self.inner_line_width * size)
            if (self.inner_line_width)
            else (OUTLINE_ZERO)
        )
        self.ids.pad._line_width = (
            (self.pad_line_width * size) if (self.pad_line_width) else (OUTLINE_ZERO)
        )
    ...
    def _update_circles(self, size):
        ...
        self._outer_diameter = (
            self._total_diameter - self._outer_line_width
        ) * self.outer_size
        ...
        self._inner_diameter = (
            self._total_diameter - self._inner_line_width
        ) * self.inner_size
