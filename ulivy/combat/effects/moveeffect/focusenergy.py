...
from ulivy.combat.effects.critchanceeffect import CritChanceEffect
from ulivy.combat.effects.statuseffect.focusenergyeffect import FocusEnergyEffect


class Focusenergy(BaseMoveEffect):
    def before_action(self):
        if "FocusEnergy" in [
            eff.name for eff in self.scene.get_effects_on_target(self.move.user)
        ]:
            self.move.fail = True
